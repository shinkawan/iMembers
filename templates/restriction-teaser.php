...
<div class="imembers-container">
    <div class="imembers-teaser-lock">
        ...
        <?php if ( ! empty( $is_logged_in ) && ! empty( $required_level ) && intval( $required_level ) === 2 ) : ?>
            <h3 style="margin:0 0 10px; font-size:18px; text-align:center;">この記事は有料会員限定です</h3>
            <p style="margin:0 0 24px; color:var(--imembers-text-muted); font-size:14px;">続きを読むには有料プランへの登録が必要です。</p>
            <a href="<?php echo esc_url( $upgrade_url ); ?>" class="imembers-btn imembers-btn-primary">有料プランを確認する</a>
        <?php else : ?>
